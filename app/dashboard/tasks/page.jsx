...
import { Plus, Play, Pause, Trash2, Clock, CheckCircle, Trophy } from 'lucide-react';
      ...
      setTimers(prev => {
        ...
        Object.keys(state.activeTimers).forEach(taskId => {
          if (state.activeTimers[taskId]) {
            updated[taskId] = Math.floor((Date.now() - state.activeTimers[taskId]) / 1000);
          }
        });
        return updated;
      });
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Create a new task to track your productivity and time.
              </DialogDescription>
            </DialogHeader>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-600 text-center mb-6">
              Create your first task to start tracking your productivity and managing your time effectively.
            </p>
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button>
              ...
              <Card key={task.id} className="hover:shadow-md transition-shadow">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor={`memo-${task.id}`}>Task Notes</Label>
                    <Textarea
                      id={`memo-${task.id}`}
                      value={task.memo || ''}
                      onChange={(e) => handleUpdateMemo(task.id, e.target.value)}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-500">
                ({filteredFinishedTasks.length} of {state.finishedTasks.length})
              </span>
            </div>
            <div className="flex space-x-2">
              <Button
                variant={completedTasksFilter === 'week' ? 'default' : 'outline'}
                size="sm"
              ...
              <Button
                variant={completedTasksFilter === 'month' ? 'default' : 'outline'}
                size="sm"
                onClick={() => setCompletedTasksFilter('month')}
              >
                Last Month
              </Button>
              <Button
                variant={completedTasksFilter === 'all' ? 'default' : 'outline'}
                size="sm"
            ...
            <Card className="border-dashed border-2 border-gray-300">
              <CardContent className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-gray-600 text-center text-sm">
                  {completedTasksFilter === 'week' && 'No tasks completed in the last week.'}
                  {completedTasksFilter === 'month' && 'No tasks completed in the last month.'}
                  {completedTasksFilter === 'all' && 'No tasks have been completed yet.'}
                </p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid gap-4">
              ...
              <Card key={task.id} className="bg-green-50 border-green-200">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {task.description && (
                        <CardDescription className="mt-1 text-green-700">{task.description}</CardDescription>
                      )}
                      <div className="mt-2 text-sm text-green-600">
                        Completed on {new Date(task.finishedAt).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                          year: 'numeric',
                          hour: '2-digit',
                ...
                <CardContent className="space-y-4">
                  ...
                  {task.memo && (
                    <div className="space-y-2">
                      <Label className="text-green-800">Task Notes</Label>
